...
import { X, UserPlus, User, Briefcase, Mail, RotateCcw, Search } from "lucide-react";
import NavbarManager from "../components/NavbarManager";
import { getAllWorkers, createWorker, getArchivedWorkers, restoreWorker } from "../api/worker";
      ...
      <div className="relative bg-bg border-t border-border rounded-t-3xl w-full max-w-sm">
        ...
        <div className="flex items-center justify-between px-5 pt-2 pb-5">
          <div>
            ...
            <p className="text-xs text-text3 mt-0.5">Worker logs in with these credentials</p>
          </div>
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full bg-bg3 text-text3">
            <X size={15} />
          </button>
        </div>
        <form onSubmit={handleSubmit} className="flex flex-col gap-3 px-5 pb-8">
          {[
            { label: "Full name", value: name, onChange: setName, type: "text", placeholder: "e.g. Ahmed Al-Rashid", icon: User },
            { label: "Email", value: email, onChange: setEmail, type: "email", placeholder: "e.g. ahmed@company.com", icon: Mail },
            { label: "Occupation", value: occupation, onChange: setOccupation, type: "text", placeholder: "e.g. Electrician", icon: Briefcase },
...
const ManagerShowListOfWorkersPage = () => {
  ...
  useEffect(() => {
    const fetchWorkers = async () => {
      try {
        const [res, res2] = await Promise.all([getAllWorkers(), getArchivedWorkers()]);
        setWorkers(res.data);
  ...
  const filtered = workers.filter((w: any) => {
    const matchesSearch = w.name.toLowerCase().includes(q) || w.occupation?.toLowerCase().includes(q);
  ...
  const handleRestore = async (e: React.MouseEvent, workerId: string) => {
    ...
    try {
      ...
      if (restored) setWorkers((prev) => [{ ...restored, isArchived: false }, ...prev]);
    } finally {
      setRestoringId(null);
    }
  };
          ...
          {(["all", "active", "pending", "archived"] as const).map((key) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-xs font-bold capitalize shrink-0 transition-colors ${
                filter === key
                  ? key === "archived" ? "bg-amber-500 text-white" : "bg-blue text-white"
        ...
        <div className="bg-bg3 border border-border rounded-2xl overflow-hidden">
          {filter === "archived" ? (
            filteredArchived.length === 0 ? (
              <p className="text-sm text-text3 text-center py-8">No archived workers</p>
            ) : (
              filteredArchived.map((worker: any) => (
                <div key={worker._id} className="flex items-center gap-3 px-4 py-3 border-b border-border last:border-b-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-text3 truncate">{worker.name}</p>
                    <p className="text-[11px] text-text3 truncate">{worker.occupation} · {worker.sites[0]?.name ?? "No site"}</p>
                  </div>
                  <button
            ...
            filtered.map((worker: any) => (
              ...
              >
                ...
                <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full shrink-0 ${
                  worker.isActivated ? "bg-green/15 text-green" : "bg-amber-500/15 text-amber-500"
                }`}>
